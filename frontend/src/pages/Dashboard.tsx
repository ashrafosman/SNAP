...
const PIE_COLORS = ['#ef4444', '#f59e0b', '#6366f1', '#22c55e', '#8b5cf6', '#06b6d4'];

const ChartTip = ({ active, payload, label }: any) => {
  if (!active || !payload?.length) return null;
  return (
    <div className="bg-[#1a1a24] border border-[#27272a] rounded-lg px-3 py-2 shadow-xl">
...
function ROICalculator({ metrics }: { metrics: OverviewMetrics }) {
  ...
  return (
    <div className="bg-[#16161e] border border-[#27272a] rounded-xl p-6">
      <div className="flex items-center gap-2 mb-1">
        <Calculator className="w-4 h-4 text-[#6366f1]" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 flex flex-col justify-center">
          <div className="mb-4">
            <div className="flex justify-between items-baseline mb-2">
              <span className="text-xs text-[#71717a]">Current Rate</span>
              <span className="text-lg font-bold text-red-400">{currentRate}%</span>
            </div>
            <div className="flex justify-between items-baseline mb-4">
              <span className="text-xs text-[#71717a]">Target Rate</span>
          ...
          <input
            ...
            className="w-full h-2 rounded-full appearance-none cursor-pointer accent-[#6366f1] bg-[#27272a]"
          />
          <div className="flex justify-between mt-2">
            <span className="text-[10px] text-[#52525b]">1%</span>
            {markers.filter(m => m.value <= currentRate).map(m => (
              <button key={m.value} onClick={() => setTargetRate(m.value)} className="text-[10px] text-[#6366f1] hover:text-white transition-colors">
                {m.label}
              </button>
            ))}
            <span className="text-[10px] text-[#52525b]">{Math.floor(currentRate)}%</span>
          </div>
          ...
          {roi.belowThreshold && (
            <div className="mt-3 bg-green-500/10 border border-green-500/30 rounded-lg px-3 py-2">
              ...
              <p className="text-[10px] text-[#71717a]">State is in the safe zone at this target rate</p>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-3 gap-4">
          <div className="bg-[#0f0f13] border border-[#27272a] rounded-xl p-4 flex flex-col items-center justify-center text-center">
            <p className="text-[10px] text-[#71717a] uppercase tracking-wider mb-1">Current Penalty (50/50)</p>
            <p className="text-lg font-bold text-red-400">${roi.currentPenalty5050.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
            <p className="text-[10px] text-[#52525b] mt-0.5">at {currentRate}% error rate</p>
            <div className="mt-2 pt-2 border-t border-[#27272a] w-full">
              <p className="text-[10px] text-[#71717a]">At target ({targetRate}%)</p>
          ...
          <div className="bg-[#0f0f13] border border-green-500/20 rounded-xl p-4 flex flex-col items-center justify-center text-center">
            <p className="text-[10px] text-[#71717a] uppercase tracking-wider mb-1">Post Oct 2026 (75/25)</p>
          ...
          <div className="bg-[#0f0f13] border border-[#6366f1]/20 rounded-xl p-4 flex flex-col items-center justify-center text-center">
            <p className="text-[10px] text-[#71717a] uppercase tracking-wider mb-1">HR1 Cost-Share Uplift</p>
            <p className="text-2xl font-bold text-[#6366f1]">+${roi.hr1Uplift.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
            <p className="text-[10px] text-[#52525b] mt-1">extra penalty avoided by acting before Oct 2026</p>
            <div className="mt-2 pt-2 border-t border-[#27272a] w-full">
              <p className="text-[10px] text-[#71717a]">Total savings at target</p>
          ...
          <div className="col-span-3 bg-[#0f0f13] border border-[#27272a] rounded-xl p-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-[#71717a]">Penalty zone: errors above {TOLERANCE}% threshold</span>
            ...
            <div className="flex justify-between mt-1">
              ...
              <span className="text-[10px] text-[#52525b]">{TOLERANCE}% federal threshold</span>
            </div>
          </div>
        </div>
      </div>
  ...
  if (loading || !metrics) {
    return (
      <div className="flex items-center justify-center h-full">
        <Activity className="w-5 h-5 text-[#6366f1] animate-pulse mr-2" />
        <span className="text-[#71717a]">Loading QC metrics...</span>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 space-y-8 max-w-[1400px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 bg-[#16161e] border border-green-500/25 rounded-xl p-5 flex items-start gap-4">
        ...
        <div className="bg-[#16161e] border border-[#27272a] rounded-xl p-5 flex flex-col justify-center items-center text-center">
          <p className="text-xs text-[#71717a] uppercase tracking-wider mb-2">Projected Penalty Savings</p>
          <p className="text-3xl font-bold text-green-400">${metrics.penalty_savings_potential.toLocaleString()}</p>
          <p className="text-xs text-[#71717a] mt-1">if HIGH+MEDIUM corrected @ new 75% rate</p>
          <div className="mt-3 w-full bg-[#27272a] rounded-full h-1.5">
            <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-2 bg-[#16161e] border border-[#27272a] rounded-xl p-6">
          <h3 className="text-sm font-semibold mb-1">Error Type Distribution</h3>
          <p className="text-xs text-[#71717a] mb-4">Top causes of SNAP QC payment errors</p>
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              ...
              <Tooltip
                contentStyle={{ background: '#1a1a24', border: '1px solid #27272a', borderRadius: 8 }}
          ...
          <div className="grid grid-cols-1 gap-1 mt-2">
            {errorTypes.slice(0, 6).map((et: any, i: number) => (
              <div key={et.error_type} className="flex items-center gap-2">
                ...
                <span className="text-xs text-[#71717a] truncate">{et.error_type}</span>
        ...
        <div className="lg:col-span-3 bg-[#16161e] border border-[#27272a] rounded-xl p-6">
          <h3 className="text-sm font-semibold mb-1">Risk Distribution by Certification Period</h3>
          <p className="text-xs text-[#71717a] mb-4">Cases bucketed by certification length (months)</p>
          <ResponsiveContainer width="100%" height={310}>
            <BarChart data={trend} margin={{ top: 5, right: 5, bottom: 5, left: -10 }}>
              ...
              <Bar dataKey="low" name="Low Risk" fill="#6366f1" stackId="a" radius={[3,3,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#16161e] border border-[#27272a] rounded-xl p-6">
          ...
          <p className="text-xs text-[#71717a] mb-4">High-risk cases highlighted</p>
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={cities.slice(0, 10)} layout="vertical" margin={{ left: 80, right: 10 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#27272a" />
              <XAxis type="number" tick={{ fill: '#71717a', fontSize: 10 }} />
              <YAxis dataKey="city" type="category" tick={{ fill: '#71717a', fontSize: 10 }} width={80} />
              <Tooltip content={<ChartTip />} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
        ...
        <div className="bg-[#16161e] border border-[#27272a] rounded-xl p-6">
          ...
          <p className="text-xs text-[#71717a] mb-4">Total QC payment correction amount</p>
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={errorTypes.filter((et: any) => et.total_exposure > 0)} margin={{ top: 5, right: 5, bottom: 40, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#27272a" />
              <XAxis dataKey="error_type" tick={{ fill: '#71717a', fontSize: 9 }} angle={-35} textAnchor="end" interval={0} />
      ...
      {/* Quick stats row */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#16161e] border border-[#27272a] rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-red-400">{metrics.flagged_cases}</p>
          <p className="text-xs text-[#71717a] mt-1">Cases with data warnings</p>
        </div>
        <div className="bg-[#16161e] border border-[#27272a] rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-amber-400">{metrics.cases_needing_review}</p>
          <p className="text-xs text-[#71717a] mt-1">Cases needing review</p>
        </div>
        <div className="bg-[#16161e] border border-[#27272a] rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-green-400">{metrics.reviewed_cases}</p>
          <p className="text-xs text-[#71717a] mt-1">Cases reviewed</p>
        </div>
        <div className="bg-[#16161e] border border-[#27272a] rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-[#6366f1]">{metrics.low_risk}</p>
          <p className="text-xs text-[#71717a] mt-1">Low-risk cases</p>
        </div>
      </div>
    </div>
  );
